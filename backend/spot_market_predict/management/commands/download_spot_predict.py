...
class Command(BaseCommand):
    help = '從 QUICK API 下載現貨市場預測資料並儲存到資料庫'
    ...
    def save_to_database(self, df: pd.DataFrame):
        ...
        existing_prices = {
            ...
            for price in QuickAreaPricePredict.objects.filter(
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="處理價格預測"):
            ...
            if key in existing_prices:
                ...
                prices_to_create.append(QuickAreaPricePredict(**price_data))

            # 批量處理以節省記憶體
            if len(prices_to_create) >= 2000:
                QuickAreaPricePredict.objects.bulk_create(prices_to_create, ignore_conflicts=True)
                prices_to_create = []
            if len(prices_to_update) >= 2000:
                QuickAreaPricePredict.objects.bulk_update(
                    prices_to_update,
